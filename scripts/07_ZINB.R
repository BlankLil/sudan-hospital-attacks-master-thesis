...
make_zinb_table <- function(models_named,
  ...
  # 2a) Extract count + zero components for each model
  ...
  # 2b) Force stable panel row ordering
  df_long <- df_long |>
    dplyr::mutate(
  ...
  # 2e) Render (Count/Zero panel grouping)
...
# Include only terms you want to show in the contemporaneous table.
# If your t-models ALSO include some lagged terms, add them here.
count_levels_t <- c(
  "(Intercept)",
...
# Include only terms you want to show in the lagged table.
# Keep terms like Log population / Log(theta) if they’re in the model.
count_levels_lag <- c(
  "(Intercept)",
